...
class DataBaseManager():
    ...
    def update_row_by_col_name(self, table_name, col_name, name_value, update_fields):
        """
        Updates a row in the specified table where the column matches the provided value.

        Parameters:
        - table_name: Name of the table to update.
        - col_name: The column to match the value against.
        - name_value: The value to match in the specified column.
        - update_fields: A dictionary of columns and values to update.
        ...
        try:
            ...
            update_clause = ', '.join([f"{key} = ?" for key in update_fields.keys()])
            query = f"UPDATE {table_name} SET {update_clause} WHERE {col_name} = ?"

            # Execute the query with the provided values
            cursor.execute(query, tuple(update_fields.values()) + (name_value,))
            ...
            if cursor.rowcount == 0:
                ...
                print(f"Row(s) with {col_name} = {name_value} updated successfully.")
                return True

        except sqlite3.Error as error:
            print(f"Error while updating row: {error}")
            return False
    ...
    def fetch_rows_by_col_name(self, table_name, col_name, name_value):
        """
        Fetches rows from the specified table where the column matches the provided value.

        Parameters:
        - table_name: Name of the table from which to fetch the rows.
        - col_name: The column to match the value against.
        - name_value: The value to match in the specified column.

        Returns:
        - A list of dictionaries representing the matching rows, or an empty list if no matches were found.
        """
        try:
            # Connect to the SQLite database
            conn = sqlite3.connect(self.db_file)
            ...
            print(f"Error while fetching rows: {error}")
            return []
